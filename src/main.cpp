...
std::vector<int> generate_ui_for_directory(std::filesystem::path &current_directory, std::filesystem::path &currently_selected_file,
	...
	std::string file_or_directory_path_str = file_or_directory_path.string();
        int oid = filesystem_browser.add_clickable_textbox(on_click, on_hover, file_or_directory_path_str, file_rects.at(i),
...
std::filesystem::path font_info_path = std::filesystem::path("assets") / "fonts" / "times_64_sdf_atlas_font_info.json";
...
FontAtlas font_atlas(font_info_path.string(), font_json_path.string(), font_image_path.string(),
    ...
    on_click = [&]() {
        if (has_extension(currently_selected_file, "obj")) {
            ...
            auto crosshair = parse_model_into_ivpnts(currently_selected_file.string(), false);
    ...
    auto smoke_texture_coordinates =
        texture_packer.get_packed_texture_coordinates(smoke_texture_path, smoke_local_uvs);
